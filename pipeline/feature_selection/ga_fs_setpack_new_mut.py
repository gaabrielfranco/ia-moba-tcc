...
class BaseProblem(object):
    def __init__(self, data, min_size, max_size, corr_threshold, maximise=True, min_possible=2, mutation_percentage=0.1):
        self.data = data
    ...
    def mutate_perc(self, individual, percentage=None):
        try:
            if percentage is None:
                percentage = self.mutation_percentage
                
            item_count, ones, zeros = self.analyse_chromossome(individual)
            n_affected_attr = int(np.round(percentage * self.max_possible))
            ...
            print('Bad individual found.')
            print(individual)
            print('item_count %d, n_affected_attr %d, n_add %d, n_rem %d, new_count %d, exceeded %d' % (item_count, n_affected_attr, n_add, n_rem, new_count, exceeded))
            
    def diversification(self, individual):
        self.mutate_perc(individual)
    ...
    def crossover(self, parent_1, parent_2):
        ...
        if len(allowed_positions):
            ...
            self.mutate_perc(parent_1)
            self.mutate_perc(parent_2)
...
def main():
    ...
    parser.add_argument('--muttype', type=float, default=0.0, help='Percentage of individual to be affected by mutation. If zero, a single attribute will be affected (default=0.0)')
    ...
    if args.muttype == 0.0:
        ...
        problem.mutation_percentage = args.muttype
        genetic.mutate_function = problem.mutate_perc
    genetic.diversification_function = problem.diversification
