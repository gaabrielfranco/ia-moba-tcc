...
def radarplot_multi(data, file_name, size_plot=(2, 5), title=None, exclude=None, label=None, show_plots=False, figsize=(3.8, 2.3)):
    ...
    for i in range(n_lines):
        for j in range(n_columns):
            ...
            values = np.concatenate((values, [values[0]]))
            lines[idx], = axarr[i, j].plot(angles, values, 'o-', linewidth=.2, markersize=1)#, color=pallete[idx])
            axarr[i, j].fill(angles, values, alpha=0.25)#, color=pallete[idx])
            axarr[i, j].set_thetagrids(angles * 180/np.pi, ["" for i in range(len(dimensions))])
...
def radarplot_comp(data, data_2, file_name, size_plot=(2, 5), title=None, exclude=None, label=None, show_plots=False, figsize=(3.8, 2.3)):
    ...
    for i in range(n_lines):
        for j in range(n_columns):
            ...
            axarr[i, j].plot(angles, values, 'o-', linewidth=.2, markersize=1)#, color=pallete[idx])
            axarr[i, j].fill(angles, values, alpha=0.25)
            axarr[i, j].plot(angles, values_2, 'o-', linewidth=.2, markersize=1)
            axarr[i, j].fill(angles, values_2, alpha=0.25)#, color=pallete[idx])
            axarr[i, j].set_thetagrids(angles * 180/np.pi, ["" for i in range(len(dimensions))])
