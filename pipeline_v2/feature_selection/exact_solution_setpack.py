...
class Problem(object):
    ...
    def manage_elite(self, individual):
        ...
        if len(self.elite) < self.elite_size:
            ...
            lower_bound = max(self.elite, key=attrgetter(
            ...
            lower_bound = min(self.elite, key=attrgetter(
                'evaluation')).evaluation
            if individual.evaluation > lower_bound:
                self.elite.pop(len(self.elite)-1)
                self.elite.append(individual)
...
class Logger(object):
    ...
    def save_log(self, solutions):
        ...
        file_tmp = self.file
        self.file = self.file[:self.file.rfind(
            '.')] + '_log' + self.file[self.file.rfind('.'):]
...
def main():
    ...
    problem = Problem(data, args.threshold, args.mins,
                      args.maxs, args.metric, args.elite_size)
