...
def main():
    ...
    parser.add_argument('perc', type=float, help='percentage of the relevance space covered by feature selection (0.0 <= perc <= 1.0)')
    ...
    for v in vls:
        if v < perc:
            originalVal = v * (np.max(sel.variances_) - np.min(sel.variances_)) + np.min(sel.variances_)
