...
Faz as requisições de todas as partidas das ligas disponíveis com mais de 20 partidas 
na API e salva tudo em arquivos json
'''
...
r = http.request(
    'GET', 'http://api.steampowered.com/IDOTA2Match_570/GetLeagueListing/v1?key=EAF3EC985E0B67D77A2B0321A2066AC7')
resultJSON = json.loads(r.data.decode('utf-8'))

print('Salvando o resultado no arquivo leagues.json...')
outfile = open('files\\leagues.json', 'w')
...
for i in ligas:
    try:
        r = http.request(
            'GET', 'https://api.steampowered.com/IDOTA2Match_570/GetMatchHistory/v1?key=EAF3EC985E0B67D77A2B0321A2066AC7&league_id=' + str(i))
        resultJSON = json.loads(r.data.decode('utf-8'))
...
for i in partidas:
    try:
        r = http.request(
            'GET', 'http://api.steampowered.com/IDOTA2Match_570/GetMatchDetails/v1?key=EAF3EC985E0B67D77A2B0321A2066AC7&match_id=' + str(i))
        resultJSON = json.loads(r.data.decode('utf-8'))
