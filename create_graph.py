...
atributes = {} #para cada player, (K, D, A, Npartidas, denies, gpm, hero_damage, hero_healing, 
               # LH, xp_p_min)
arquivos = glob.glob('/home/gabriel/Desktop/Arquivos/Partidas/*.json')
        ...
        if (txt_json['result']['human_players'] == 10):
                ...
                for k in range (0, len(txt_json['result']['players'])):
                    ...
                    if jogador in atributes:
                        ...
                        txt_json['result']['players'][k]['deaths'], atributes[jogador][2] + txt_json['result']['players'][k]['assists'], \
                        atributes[jogador][3] + 1, atributes[jogador][4] + txt_json['result']['players'][k]['denies'], atributes[jogador][5] \
                        + txt_json['result']['players'][k]['gold_per_min'], atributes[jogador][6] + txt_json['result']['players'][k]['hero_damage'], \
                        atributes[jogador][7] + txt_json['result']['players'][k]['hero_healing'], atributes[jogador][8] + txt_json['result']['players'][k]['last_hits'], \
                        ...
                        txt_json['result']['players'][k]['assists'], 1, txt_json['result']['players'][k]['denies'], txt_json['result']['players'][k]['gold_per_min'], \
...
print('Salvando em files/atributes.txt')
arq = open('files/atributes.txt', 'w')
for i in atributes:
    arq.writelines(str(i) + ' ' + str(atributes[i][0]) + ' '+ str(atributes[i][1]) + ' ' + str(atributes[i][2])+ ' ' + str(atributes[i][3]) \
    + ' ' + str(atributes[i][4]) + ' ' + str(atributes[i][5]) + ' ' + str(atributes[i][6]) + ' ' + str(atributes[i][7]) + ' ' + str(atributes[i][8]) \
    + ' ' + str(atributes[i][9]) + '\n')
arq.close()


arq = open('files/invalids_matches.txt', 'w')
for i in lista:
