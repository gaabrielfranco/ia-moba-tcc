...
def classification(k, data_all, data_kda):
    ...
    for i in range(0, 10):
        ...
        '''file_all.write('Labels da Iteracao ' + str(i) + ':\n[')
        for a, b in enumerate(labels):
            if (a + 1 == len(labels)):
                file_all.write(str(b) + ']\n')
            else:
                file_all.write(str(b) + ', ')'''
    ...
    for i in range(0, 10):
        ...
        '''file_kda.write('Labels da Iteracao ' + str(i) + ':\n[')
        for a, b in enumerate(labels):
            if (a + 1 == len(labels)):
                file_kda.write(str(b) + ']\n')
            else:
                file_kda.write(str(b) + ',')'''
...
def main():
    ...
    for l in fp:
        ...
        for i, p in enumerate(partes):
            partes[i] = int(p)
        data_all.append(partes[1:6] + [partes[9]])
